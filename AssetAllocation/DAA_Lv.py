...
def DAA_LV_MAIN(start_day, end_day, df_DB, Sample):

    Offensive = ['SPY','IWM','QQQ','SSO','QLD','UWM','VGK','EWJ','VWO','VNQ','GSG','GLD','TLT','HYG','LQD']
...
# monthly profit & accumulated profit 
    ...
    df_DAA_T = Call_AA_Today_2(df_DAA,'DAA_LV',DAA_T_List)
...
def select_DAA_asset(x, mom_col_list_Off, mom_col_list_Def):
    ...
    if x['VWO_M'] > 0 and x['BND_M'] > 0:
        ...
        if x[x == momentum1].index[0][:3] == 'SPY':
            asset['DAA_ASSET1'] = 'SSO'
        elif x[x == momentum1].index[0][:3] == 'QQQ':
            asset['DAA_ASSET1'] = 'QLD'
        elif x[x == momentum1].index[0][:3] == 'IWM':
            asset['DAA_ASSET1'] = 'UWM'
        else:
            asset['DAA_ASSET1'] = x[x == momentum1].index[0][:3]

        if x[x == momentum2].index[0][:3] == 'SPY':
            asset['DAA_ASSET2'] = 'SSO'
        elif x[x == momentum2].index[0][:3] == 'QQQ':
            asset['DAA_ASSET2'] = 'QLD'
        elif x[x == momentum2].index[0][:3] == 'IWM':
            asset['DAA_ASSET2'] = 'UWM'
        else:
            asset['DAA_ASSET2'] = x[x == momentum2].index[0][:3]

        if x[x == momentum3].index[0][:3] == 'SPY':
            asset['DAA_ASSET3'] = 'SSO'
        elif x[x == momentum3].index[0][:3] == 'QQQ':
            asset['DAA_ASSET3'] = 'QLD'
        elif x[x == momentum3].index[0][:3] == 'IWM':
            asset['DAA_ASSET3'] = 'UWM'
        else:
            asset['DAA_ASSET3'] = x[x == momentum3].index[0][:3]

        if x[x == momentum4].index[0][:3] == 'SPY':
            asset['DAA_ASSET4'] = 'SSO'
        elif x[x == momentum4].index[0][:3] == 'QQQ':
            asset['DAA_ASSET4'] = 'QLD'
        elif x[x == momentum4].index[0][:3] == 'IWM':
            asset['DAA_ASSET4'] = 'UWM'
        else:
