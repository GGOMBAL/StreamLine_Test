...
def VAA_LV_MAIN(start_day, end_day, df_DB, Sample):

    # Makeup Universe
    #Offensive = ['SPY','VEA','EEM','AGG'] 
    Offensive = ['SSO','EFO','EET','AGG','SPY','VEA','EEM']
...
# monthly profit & accumulated profit 
    ...
    df_VAA_T = Call_AA_Today_2(df_VAA,'VAA_LV',VAA_T_List)
...
def select_VAA_asset(x):
    ...
    #if x['SPY_M'] > 0 and x['EFO_M'] > 0 and x['EET_M'] > 0 and x['AGG_M'] > 0:
    #    max_momentum = max(x['SSO_M'],x['EFO_M'],x['EET_M'],x['AGG_M'])     
    
    #else :
    #    max_momentum = max(x['LQD_M'],x['SHY_M'],x['IEF_M'])
    
    if x[x == max_momentum].index[0][:3] == 'SPY':
        asset['ASSET1'] = 'SSO'
        asset['PRICE1'] = x[asset['ASSET1']]
    elif x[x == max_momentum].index[0][:3] == 'VEA':
        asset['ASSET1'] = 'EFO'
        asset['PRICE1'] = x[asset['ASSET1']]
    elif x[x == max_momentum].index[0][:3] == 'EEM':
        asset['ASSET1'] = 'EET'
        asset['PRICE1'] = x[asset['ASSET1']]
    else:
